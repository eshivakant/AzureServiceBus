...
namespace LuceneWrapper
{
    ...
    /// </summary>
    public class IndexedDocument<T> : IDocument
    {
        private string id;
...
#pragma warning restore 649
        ...
        protected IndexedDocument(string typeString)
        {
        ...
        public string SearchableString
        {
            ...
            set
            {
                _searchableString = value;
                AddParameterToDocumentNoStoreParameter("SearchableString", _searchableString);
            }
        }


        public static explicit operator IndexedDocument<T>(T obj)
        {
            var doc = new IndexedDocument<T>();

            doc.SearchableString = GetString(obj);

            PropertyInfo[] props = obj.GetType().GetProperties();
            foreach (PropertyInfo prop in props)
            {
                var isIndex = prop.GetCustomAttributes(true).Any(a=>a.GetType()==typeof(IndexKey));
                if (isIndex)
                {
                    doc.Id = prop.GetValue(obj).ToString();                    
                    break;
                }
            }
